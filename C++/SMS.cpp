...
void statistics()    //8.学生信息统计工具，计算最高分、最低分、平均分、方差
{
   ...
   float maxc = m[0].less.chinesegrade,                        //语文最大值
         ...
         max_chinesegrade = m[0].less.chinesegrade,
         max_englishgrade = m[0].less.englishgrade,
         max_mathgrade    = m[0].less.mathgrade,
         max_num = m[0].num,
         min_chinesegrade = m[0].less.chinesegrade,
         min_englishgrade = m[0].less.englishgrade,
         min_mathgrade    = m[0].less.mathgrade,
         min_num = m[0].num;;

   strcpy(max_class, m[0].clas);
   strcpy(max_name, m[0].name);
   strcpy(min_class, m[0].clas);
   strcpy(min_name, m[0].name);
